...
public class StampboardLikeService {
	...
	public Integer likeStamp(long uid, long sid) {
		if(stampboardLikeRepository.findByUserIdAndStampboardId(userRepository.findByUserId(uid), stampboardRepository.findById(sid).get())!=null){
			return 0;
		}else {
			StampboardLike stampboardLike = StampboardLike.builder().userId(userRepository.findByUserId(uid)).stampboardId(stampboardRepository.findById(sid).get()).build();
	...
	//유저가 좋아요한 스탬프 리스트 반환
	public List<StampboardLike> likeList(long uid){
		List<StampboardLike> list = stampboardLikeRepository.findAllByUserId(userRepository.findByUserId(uid));
