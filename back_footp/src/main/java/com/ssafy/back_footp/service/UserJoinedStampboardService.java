...
public class UserJoinedStampboardService {
	...
	// 진행중인 스탬푸 삭제(취소)
	@Transactional
	public Integer deleteStamp(long uid, long usid) {
		
		userJoinedStampboardRepository.deleteById(usid);
	...
	//스탬푸 도장 찍기 + 스탬푸 도장 완료 
	@Transactional
	public Integer clearStamp(long usid, long mid) {
		
		UserJoinedStampboard userJoinedStampboard = userJoinedStampboardRepository.findById(usid).get();
		Stampboard stampboard = stampboardRepository.findById(userJoinedStampboard.getStampboardId().getStampboardId()).get();
		User user = userRepository.findByUserId(userJoinedStampboard.getUserId().getUserId());
		
		
		if(!userJoinedStampboard.getUserjoinedstampboardIsclear1() && stampboard.getStampboardMessage1().getMessageId()==mid) {
		...
		}else if(!userJoinedStampboard.getUserjoinedstampboardIsclear2() && stampboard.getStampboardMessage2().getMessageId()==mid) {
		...
		}else if(!userJoinedStampboard.getUserjoinedstampboardIsclear3() && stampboard.getStampboardMessage3().getMessageId()==mid) {
		...
		if(userJoinedStampboard.getUserjoinedstampboardIsclear1() && userJoinedStampboard.getUserjoinedstampboardIsclear2() && userJoinedStampboard.getUserjoinedstampboardIsclear3()) {
